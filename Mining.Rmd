...
Rob_extraction:	Was the data abstraction from each study appropriate? Data extraction must have been performed by at least two reviewers.
...
pdf_text <- pdf_text("sample_pdf.pdf")

# Concatenate the text elements into a single character vector
text_vector <- paste(pdf_text, collapse = " ")

data.frame(text=text_vector) %>% 
  mutate(RoB=ifelse(grepl(dict_rob,text),"yes","no")) %>% 
  select(RoB)
```

```{r}
names<-list.files("Included-fulltexts-PDF")
data<-data.frame()
...
for(i in names){
  ...
  mutate(title=i,RoB=ifelse(grepl(dict_rob,text),"yes","no")) %>% 
  select(RoB)
